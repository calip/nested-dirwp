...
class StaffDirectory {
  ...
  static function register_post_types() {
    ...
    add_action( 'restrict_manage_posts', array('StaffDirectory', 'restrict_listings_by_product_cat' ));
    add_filter( 'pre_get_posts', array('StaffDirectory', 'wpse_filter_child_cats' ));
  }

  
  static function wpse_filter_child_cats( $query ) {
...
static function restrict_listings_by_product_cat() {
	global $typenow;
	global $wp_query;
	if ( $typenow == 'staff' ) {
		$taxonomy         = 'staff_category';
		$current_taxonomy = get_taxonomy( $taxonomy );
		$selected         = isset( $wp_query->query['staff_category'] ) ? $wp_query->query['staff_category'] : '';
		wp_dropdown_categories(
			array(
				'walker'          => new ic_walker_tax_slug_dropdown(),
				'value'           => 'slug',
				'show_option_all' => __( "All", "staff-category" ) . ' ' . $current_taxonomy->label,
				'taxonomy'        => $taxonomy,
				'name'            => 'staff_category',
				'orderby'         => 'name',
				'selected'        => $selected,

/**
 * Javascript to help the Nested Directory in the WordPress admin
 * This file is part of the Nested Directory plugin for WordPress
 */
 ...
 * @param filename
 */
function nesteddir_exportTableToCSV(table, filename)
{
    ...
    {
        window.navigator.msSaveOrOpenBlob(new Blob([csv], {type: "text/plain;charset=utf-8;"}), "nesteddir_export.csv")
    }
    else
    {
...
function nesteddir_savePreference(name, value)
{
    var expires = "";
...
function nesteddir_readPreference(name)
{
    var nameEQ = name + "=";
...
jQuery(document).ready(function()
{
    ...
    if(jQuery(".nesteddir_import_page #import-upload-form").length > 0)
    {
        jQuery(".nesteddir_import_page #import-upload-form").prepend("<p><input type='checkbox' name='use_utf8_import' value='1' id='use_utf8_import'> <label for='use_utf8_import'>" + nesteddir_translation.use_utf8_import + "</label></p>");
        jQuery(".nesteddir_import_page #import-upload-form").prepend("<p><input type='checkbox' name='empty_dir_on_import' value='1' id='empty_on_import'> <label for='empty_on_import'>" + nesteddir_translation.empty_directory_on_import + "</label></p>");
    }
    ...
    jQuery("#input_compact").on("click", function(e)
    {
        ...
        nesteddir_savePreference("wp-plugin-nd-add_form", "compact");
    });

    jQuery("#input_extensive").on("click", function(e)
    {
        jQuery("#add_description, #add_published, #add_submitter").show();
        nesteddir_savePreference("wp-plugin-nd-add_form", "extensive");
    });

    jQuery('.nesteddir_confirmdelete').click(function()
    {
        var delete_directory = confirm(nesteddir_translation.delete_question);
    ...
    var pref = nesteddir_readPreference("wp-plugin-nd-add_form");
    if(pref != null)
    {
    ...
    jQuery("#add_form_ajax_submit").val("nesteddir_ajax_names");
    ...
    jQuery(".toggle_published").on("click", function(e)
    {
        ...
        jQuery.ajax({
            url: "admin-ajax.php",
            type: "POST",
            data: { action: "nesteddir_switch_name_published_status", name_id: name_id }
    ...
    jQuery("#export_nesteddir_names_button").on('click', function (event)
    {
        nesteddir_exportTableToCSV.apply(this, [jQuery('#export_names'), 'nesteddir_export.csv']);
    });
});

...
function nesteddir_db_tables()
{
    ...
    $nesteddir_table_queries = array("
        CREATE TABLE $nesteddir_table_directory (
            ...
            UNIQUE KEY id (id),
            PRIMARY KEY (id));",
        "CREATE TABLE $nesteddir_table_directory_name (
            id INT( 11 ) NOT NULL AUTO_INCREMENT,
    ...
    dbDelta( $nesteddir_table_queries );

    update_option("nesteddir_db_version", $nesteddir_db_version);
}
 ...
 */
function nesteddir_db_post_update_actions()
{
    global $wpdb;
    global $nesteddir_db_version;
    global $nesteddir_table_directory;
    ...
    $convert_dirs = "ALTER TABLE $nesteddir_table_directory CONVERT TO CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci';";
    $wpdb->query($convert_dirs);

    $convert_names = "ALTER TABLE $nesteddir_table_directory_name CONVERT TO CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci';";
    $wpdb->query($convert_names);

    $convert_opt = "UPDATE $nesteddir_table_directory SET `show_all_names_on_index`=1 WHERE `show_all_names_on_index` IS NULL;";
    $wpdb->query($convert_opt);

    $wpdb->show_errors = false;
    $wpdb->suppress_errors = true;
    $make_index = "ALTER TABLE $nesteddir_table_directory_name ADD INDEX `dir_idx` (`directory` DESC);";
    $wpdb->query($make_index);
    $make_index2 = "ALTER TABLE $nesteddir_table_directory_name ADD INDEX `published_idx` (`published` DESC);";
 ...
 */
function nesteddir_db_install_demo_data()
{
    global $wpdb;
    global $nesteddir_table_directory;
    global $nesteddir_table_directory_name;
    ...
    $wpdb->query(sprintf("SELECT * FROM " . $nesteddir_table_directory));
    if($wpdb->num_rows === 0)
    {
        $wpdb->insert($nesteddir_table_directory, array(
            'id'                        => 1,
        ...
        $wpdb->insert($nesteddir_table_directory_name, array(
            'directory'     => 1,
            'name'          => 'Navi',
            'letter'        => 'N',
